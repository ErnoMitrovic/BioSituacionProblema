...
La situación actual de COVID-19 poco a poco presenta un menor riesgo en el mundo, gracias a la aplicación de las diferentes dosis de vacunación. Lo cual implica cada vez  un menor número de contagios y por ende un menor número de defunciones. A pesar de esta evidente mejora, los contagios siguen aún sin detenerse. Un claro ejemplo es el continente Europeo, el cual sigue manteniéndose como el continente con más casos reportados al día de hoy y de igual forma Estados Unidos como uno de los países con mayor número de contagios.  
...
barplot(matrix(c(B$a, B$c, B$g, B$t), nrow = 10), beside = TRUE, col = colors, las =2, names.arg = c("A", "C", "G", "T"))
